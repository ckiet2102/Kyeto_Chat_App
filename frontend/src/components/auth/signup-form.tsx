...
export function SignupForm({ className, ...props }: React.ComponentProps<"div">) {
  ...
  const handleVerifyOTP = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await authService.verifyRegistrationOTP(registeredEmail, fullOtp);
  ...
  const handleResendOTP = async () => {
    if (!canResend || verifyingOtp) return;
    try {
      setVerifyingOtp(true);
      const res = await authService.resendRegistrationOTP(registeredEmail);
      toast.success(res.message || "Đã gửi lại mã OTP xác thực đăng ký!");
      setResendTimer(60);
      setCanResend(false);
    } catch (err: any) {
    ...
    >
      <Card className="overflow-hidden p-0 border border-amber-500/30 dark:border-amber-500/25 shadow-2xl shadow-amber-500/10 rounded-2xl bg-card/90 backdrop-blur-xl">
        <CardContent className="grid p-0 md:grid-cols-2">
            ...
            >
              <div className="flex flex-col gap-5">
                ...
                <div className="flex flex-col items-center text-center gap-2">
                  ...
                  <h1 className="text-3xl font-extrabold bg-gradient-to-r from-amber-200 via-yellow-400 to-amber-500 bg-clip-text text-transparent tracking-tight drop-shadow-sm">Tạo tài khoản Kyeto</h1>
          ...
          {/* STEP 2: Registration OTP Verification Screen */}
          {step === 2 && (
            <form
            ...
            >
              <div className="text-center space-y-2">
                <div className="mx-auto size-14 rounded-2xl bg-amber-500/10 border border-amber-500/30 flex items-center justify-center text-amber-500 shadow-inner">
                  <ShieldCheck className="size-8 text-amber-400 animate-pulse" />
                </div>
                <h1 className="text-2xl font-extrabold bg-gradient-to-r from-amber-200 via-yellow-400 to-amber-500 bg-clip-text text-transparent tracking-tight">
                  Xác Thực Email Đăng Ký
                </h1>
                <p className="text-xs text-muted-foreground">
                  Mã OTP 6 số đã được gửi đến email <strong>{registeredEmail}</strong>
                </p>
              </div>
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  ...
                  <button
                    ...
                    disabled={!canResend || verifyingOtp}
                  ...
                  >
                    <RefreshCw className={`size-3 ${verifyingOtp ? "animate-spin" : ""}`} />
                ...
                <div className="flex justify-between gap-1.5 sm:gap-2">
                  {otpDigits.map((digit, index) => (
                    <input
                      ...
                      className="size-10 sm:size-11 text-center text-lg font-bold rounded-xl border border-amber-500/30 bg-background/50 focus:border-amber-500 focus:ring-1 focus:ring-amber-500 outline-none text-amber-400 shadow-inner transition-all"
                    />
                  ))}
                </div>
              </div>
              ...
              <Button
                ...
                className="w-full bg-gradient-to-r from-amber-400 via-yellow-500 to-amber-600 text-slate-950 font-bold shadow-lg shadow-amber-500/25 hover:shadow-amber-500/40 hover:scale-[1.01] active:scale-[0.99] transition-all duration-300 rounded-xl h-11 text-sm tracking-wide cursor-pointer border border-amber-300/40"
              >
                {verifyingOtp ? "Đang xác thực..." : "Xác Thực OTP & Kích Hoạt Tài Khoản"}
              </Button>
              ...
              <div className="text-center pt-2">
                <button
                  type="button"
                  onClick={() => setStep(1)}
                  className="inline-flex items-center text-xs text-amber-500 hover:text-amber-400 font-semibold gap-1 transition-colors cursor-pointer"
                >
                  <ArrowLeft className="size-3.5" /> Quay lại Form Đăng ký
                </button>
              </div>
            </form>
          )}
